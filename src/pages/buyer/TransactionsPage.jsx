...
export default function TransactionsPage() {
  ...
  const fetchTransactions = async (p = 1) => {
    ...
    try {
      ...
      const { data } = await transactionAPI.buyerTransactions(params);
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left px-4 py-3 font-medium hidden sm:table-cell">Store</th>
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3">
                        <Link to={`/orders/${tx.orderId}`} className="font-medium text-rosewood-600 hover:underline flex items-center gap-1">
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <p className="text-sm">{tx.seller?.storeName || tx.seller?.fullName}</p>
                      </td>
