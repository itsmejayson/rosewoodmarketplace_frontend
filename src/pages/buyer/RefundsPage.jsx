      ...
      {refunds.length === 0 ? (
        ...
        <div className="space-y-4">
          {refunds.map((refund) => {
            ...
            const statusColor =
              STATUS_STYLES[refund.status] ?? 'bg-gray-100 text-gray-800';
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      {orderId ? (
                        <Link
                          to={`/orders/${orderId}`}
                          className="text-rosewood-600 hover:underline font-medium"
                        >
                          {orderNumber}
                        </Link>
                      ) : (
