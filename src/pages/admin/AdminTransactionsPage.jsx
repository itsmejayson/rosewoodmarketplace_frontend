...
export default function AdminTransactionsPage() {
  ...
  const fetchTransactions = async (p = 1) => {
    ...
    try {
      ...
      const { data } = await transactionAPI.allTransactions(params);
  ...
  useEffect(() => { setPage(1); fetchTransactions(1); }, [paymentStatus, paymentMethod]);
      ...
      <div className="flex gap-3 flex-wrap">
        <Select value={paymentStatus} onValueChange={setPaymentStatus}>
          ...
          <SelectContent>
            ...
            {['PENDING','PAID','FAILED','VERIFIED','REJECTED','REFUNDED'].map((s) => (
                ...
                {transactions.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${PAYMENT_STATUS_COLOR[tx.paymentStatus] || 'bg-gray-100 text-gray-600'}`}>
                          {tx.paymentStatus}
                        </span>
                      </td>
